...
class CheckWorker
  ...
  def perform(domain)
    ...
    # catching exception
      ...
      if exception.index("SSL_connect")
        current_state = exception.sub(/.*: /, "")
      else
    ...
    if current_state == ""
      if expire_days < 7
        current_state = "cerificate expires less than in 1 week"
      elsif expire_days < 14
        current_state = "cerificate expires less than in 2 week"
      else
